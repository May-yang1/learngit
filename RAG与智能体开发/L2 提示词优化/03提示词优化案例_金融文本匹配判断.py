...
questions = [
    ...
    ("股票市场今日上涨,投资者乐观.","持续上涨的市场让投资者感到满意.")
]
"""
...
"""
message = [{"role":"system","content":f"你帮我完成文本匹配,我给你2个句子,被[]包围,你判断它们是否匹配,回答是或不是,请参考如下示例:"}]

for key,value in examples_data.items():
    for t in value:
        message.append({"role": "user","content":f"句子1:[{t[0]}],句子2:[{t[1]}]"})
        message.append({"role":"assistant","content":key})

for q in questions:
    response = client.chat.completions.create(
        model = "qwen3-max",
        messages = message + [{"role":"user","content":f"按照上述示例,回答这2个句子的情况.句子1:[{q[0]}]句子2[{q[1]}]"}]
    )
